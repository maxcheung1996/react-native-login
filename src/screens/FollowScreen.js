...
import { Avatar, IconButton, Card, Text, Button, Provider, Modal, Portal, TextInput, Checkbox } from 'react-native-paper';
...
const FollowScreen = ({ navigation }) => {
    ...
    const [eformResultDetailGuid, setEformResultDetailGuid] = useState("");
    ...
    const showModal = (eformResultDetailGuid) => {
        ...
        tmp_eformResultDetailGuid = checkListAllSubDetail.filter((v,i) => {return v.eformResultDetailGuid === eformResultDetailGuid})
    ...
    const addToFollowList = (section) => {
        if (JSON.stringify(followList).indexOf(section) > -1) {
            ...
            let tmp_section = checkListAllSubDetail.filter((v, i) => { return v.eformResultDetailGuid === section })
    ...
    const removeItem = (eformResultDetailGuid) => {
        ...
        for (const item of tmp_followList) {
            if(item.filter((v, i) => { return v.eformResultDetailGuid !== eformResultDetailGuid }).length > 0) 
                new_followList.push(item.filter((v, i) => { return v.eformResultDetailGuid !== eformResultDetailGuid }))
        }
    ...
    return (
        <>
        <SubModal hideModal={hideModal} visible={visible} checkListSubDetail={checkListSubDetail} />
        <View style={style.container}>
            <View style={style.subheader}>
                <View style={{ paddingLeft: 5, flex: 1, alignItems: 'center', justifyContent: 'center' }}>
            ...
            <FlatList
                data={followList}
                renderItem={(item) => renderItem(item.item, userInfo, removeItem, showModal)}
                keyExtractor={item => item.eformResultSubDetailGuid}
            />
...
const renderItem = (item, userInfo, removeItem, showModal) => {
    const LeftContent = props => <Avatar.Icon {...props} icon={() => <MaterialIcons
        color={'#fff'}
        name="construction"
        size={22}
    />} />
    ...
    return (
        <Card style={style.card} onPress={() => showModal(item[0].eformResultDetailGuid)}>
            <Card.Title title={item[0].sectionTitle} subtitle={item[0].sectionSubtitle} left={LeftContent} right={() => <Card.Actions>
                <IconButton
                    backgroundColor={'#fff'}
                    icon={('trash-can')}
                    iconColor={'red'}
            ...
            <Card.Content>
                <View style={{ flex: 1, flexDirection: 'row' }}>
                    <View style={{ flex: 1 }}>
                        <Text style={{ alignSelf: 'flex-start', color: 'grey', fontWeight: 'bold', fontSize: 12 }}>建立者: {userInfo.fullname}</Text>
                    </View>
                    <View style={{ flex: 1 }}>
                        <Text style={{ alignSelf: 'flex-end', color: 'grey', fontWeight: 'bold', fontSize: 12 }}>最後更新: {convertDateString(item[0].updatedTimestamp)}</Text>
                    </View>
                </View>
            </Card.Content>
        </Card>
...
const SubModal = ({hideModal, visible, checkListSubDetail}) => {
    ...
    return (
        <Portal>
          <Modal visible={visible} onDismiss={hideModal} contentContainerStyle={containerStyle}>
            ...
            {React.Children.toArray(checkListSubDetail.map((v, i) => {
                    ...
                    return (
                      <View>
                        <TextInput
                          ...
                          mode={'outlined'}
                          label={v.subHeader1}
                          right={<TextInput.Icon size={13} icon="eraser" />}
                    ...
                    return (
                      <>
                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                          ...
                          <Checkbox.Android status={v.subAns1 === "1" ? 'checked' : 'unchecked'} onPress={() => (console.log(v.subAns1))} />
                        </View>
                      </>
                    );
